...
4. **頁碼感知 RAG 顧問 (Page-Aware RAG)**：將 MariaDB 結構化財務指標 (ROE)、輿情情緒指標與 PDF 實體頁面內容深度融合，於 Chatbot 生成的繁中回答句尾強制標記 `[p.X]` 與 `[資料庫]` 引用標籤。
...
flowchart TD
    ...
    subgraph RAGConsultant["4. RAG 智能顧問問答系統 (RAG Chatbot)"]
        D1[使用者提問] --> D2[chat_api.php]
        C1 --> D2
        D2 --> D3[提問 N-gram 分詞與 Bigram 提取]
        D3 --> D4[頁面關鍵字詞命中評分]
        D4 --> D5[篩選 Top 40 頁面並依頁碼排序]
        D5 --> D6[上下文整合與 Prompt 組裝]
        D6 --> D7[調用 Ollama: qwen2.5:7b]
        D7 --> D8[生成繁體中文回覆 + 來源引用標記]
    end
```
...
| [chat_api.php](file:///c:/xampp/htdocs/eco_sys/api/chat_api.php) | RAG 檢索逻辑：中文 N-gram/Bigram 分詞、頁面詞頻排序、Facts 融合與 Ollama 對接 | PHP |
